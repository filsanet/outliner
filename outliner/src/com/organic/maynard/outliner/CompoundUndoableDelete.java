...
public class CompoundUndoableDelete implements Undoable {

	private Vector primitives = new Vector(1,25);
	Node parent = null;
	
	// The Constructors
	public CompoundUndoableDelete(Node parent) {
		this.parent = parent;
	}
	...
	// Accessors
	public void addPrimitive(PrimitiveUndoableDelete primitive) {
		primitives.addElement(primitive);
	}
	
	// Undoable Interface
	public void undo() {
		Node youngestNode = ((PrimitiveUndoableDelete) primitives.elementAt(0)).getNode();
		...
		for (int i = 0; i < primitives.size(); i++) {
			((PrimitiveUndoableDelete) primitives.elementAt(i)).undo();
		}
	...
	public void redo() {
		...
		for (int i = primitives.size() - 1; i >= 0; i--) {
			((PrimitiveUndoableDelete) primitives.elementAt(i)).redo();
		}
		...
		//newSelectedNode.expandAllAncestors();
